...
## Latest
v1.0.89 | 4bcc33e4 | 2026-05-30 | versionCode 97 | Internal 🚧 | **Wallpaper Weekly reset countdown — per-engine Claude/Codex/Both (parity with 5h)** (#3022). PR #3018 v1.0.88 rollup 漏實作 Weekly 那條 per-engine pipeline (整條 hardcoded local Monday 00:00 + 字串模板無 engine slot)；現在 Weekly 跟 5h 走同一個 isEngineVisibleForReset filter，讀 server-supplied epoch (snapshot.claude.live.rateLimits.sevenDay.resetsAt + snapshot.codex.rateLimits.sevenDayResetsAt)，按 Claude/Codex toggle 顯示 1 或 2 條，legacy 本地 Monday line 只在 snapshot==null 或兩邊 sevenDay raw 都缺時 fallback (per #6 Codex design ack Q1-Q4 + #1 Mac_F supervisor cross-review hardened gate to split raw-presence from future-only filtering at PR review). 新字串 wallpaper_reset_window_weekly_next_per_engine 補在 values/ + values-zh-rTW/ (parity with existing 2-locale weekly_next scope; Hermes follow-up for remaining 14 locales).
