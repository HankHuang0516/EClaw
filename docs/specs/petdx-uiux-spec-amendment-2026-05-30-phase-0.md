...
2. **At the `POST /api/bind` leg of a rebind cycle** when `character` changes AND the entity has no user-set custom avatar (i.e. `avatar` is `null` or a system-default emoji per `_isCharacterDefaultAvatar()` — see `backend/index.js`). Rebind in the live system happens as `DELETE /api/entity` (or `DELETE /api/device/entity`) → `POST /api/bind`; the bind hook sees that prior record as `ctx.previousEntity` and uses it to distinguish bind vs rebind (see §0.3). There is **no** separate `/api/entity/rebind` endpoint.
...
- `/api/bind` — primary bind endpoint (accepts `code` + optional `name` only); per `backend/index.js:7000` it does not classify bind vs rebind itself. Rebind in the live system appears to this hook as a `DELETE /api/entity` (or `DELETE /api/device/entity`) → `POST /api/bind` cycle, and the hook distinguishes it by receiving the prior record as `ctx.previousEntity`.
...
| 0.6     | 2026-05-31 | LOBSTER #2   | Per #6 fifth review: §0.1 trigger #2 rebind detection wording aligned with §0.3 (`ctx.previousEntity`, not endpoint-local counter state); §0.3 prologue bullet for `/api/bind` states it does not classify bind vs rebind itself — the hook distinguishes the `DELETE → POST /api/bind` cycle by receiving the prior record as `ctx.previousEntity`. PR title + body bumped to v0.6.|
