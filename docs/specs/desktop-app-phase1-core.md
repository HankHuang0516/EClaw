...
| EClaw Codex | Process enumeration + identity handshake | `claude --version` + presence of `~/.openclaw` session dir → match expected version/identity (no secret read) |
...
2. For each endpoint type:
   ...
   b. Codex: process enumeration → `claude --version` identity handshake (NEVER read credentials/passwords from the CLI)
