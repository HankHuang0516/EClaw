...
取得方式：Eclaw Web Portal → 右上角頭像 → **Settings** → Device Secret 列 → 點眼睛圖示顯示 → 複製。
...
> 若 bot 是透過 Channel 綁定的，也可以用 `channel_api_key` 替代 `botSecret` 進行 API 認證。
...
Eclaw 有一份 AI 指令說明書，告訴 Claude 有哪些 API 可用。說明書網址：

```
https://eclawbot.com/api/skill-doc
```

### 方法 A：Claude Code CLI（最簡單）

直接對 Claude 說，讓它自己用 WebFetch 抓取：

```
請用 WebFetch 讀取 https://eclawbot.com/api/skill-doc，然後幫我控制我的 OpenClaw bot。
```

Claude Code 有 WebFetch 工具，會直接讀取說明書，不需要手動複製貼上。

### 方法 B：Claude.ai + 貼文字 / 上傳檔案

1. 在瀏覽器開啟說明書連結，複製全文，貼入 Claude.ai 對話
2. 或點擊 📎 上傳 `.md` 檔案
...
> **提示**：若該 bot 是透過 Channel 綁定的，可將 `botSecret` 欄位替換為 `channel_api_key`。
...
> **提示**：Channel 綁定的 bot 可將 `botSecret` 替換為 `channel_api_key`。
...
> bot 的說明文件（E-claw_mcp_skill.md）中，排程 API（`POST /api/bot/schedules`）沒有足夠清楚地說明。同時，`POST /api/schedules` 需要 deviceSecret（bot 沒有），但 `POST /api/bot/schedules` 只需要 botSecret（或 channel 綁定 bot 的 `channel_api_key`）——這個差異文件沒講清楚，導致 bot 用了錯誤的 API。
...
| 交辦任務          | 「叫 bot 去做 \_\_\_」                |
| 更新 bot 顯示訊息 | 「讓 bot 顯示 '\_\_\_'」              |
| 設定排程          | 「每天/每小時 讓 bot 自動做 \_\_\_」  |
| 查看排程          | 「列出我設定的所有排程」              |
| 取消排程          | 「取消 '\_\_\_' 這個排程」            |
| 查看記錄          | 「bot 最近說了什麼？」                |
...
**Q: Bot Secret 是什麼？用戶需要它嗎？**
A: Bot Secret 是 bot 在綁定時收到的認證金鑰，由 bot 自己持有，用於 bot 向 Eclaw 伺服器回報狀態。用戶（人類）和 Claude（作為指揮官）**不需要** Bot Secret，只需 Device ID + Device Secret + Entity 編號。若 bot 是透過 Channel 綁定的，bot 也可以用 `channel_api_key` 替代 `botSecret` 進行認證。

**Q: Claude 下的指令 bot 沒有反應？**
A: 確認以下幾點：

- Bot 是否已綁定（`isBound: true`）
- Bot Secret（或 Channel 綁定 bot 的 `channel_api_key`）是否正確
...
_文件版本：2026-03-07 | 適用系統：Eclaw v1.1+_
