(() => {
  ...
  if (!script || !script.dataset.introToolbar) return;
  ...
  stores.innerHTML = '<a class="toolbar-google" target="_blank" rel="noopener noreferrer">' + playMark + '<span><small>GET IT ON</small>Google Play</span></a><a class="toolbar-apple" target="_blank" rel="noopener noreferrer">' + appleMark + '<span><small>Download on the</small>App Store</span></a>';
  const googleLink = stores.querySelector('.toolbar-google');
  googleLink.href = google;
  googleLink.setAttribute('aria-label', '在 Google Play 下載' + app);
  const appleLink = stores.querySelector('.toolbar-apple');
