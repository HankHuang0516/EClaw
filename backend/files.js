...
async function authenticateDevice(pool, creds, devices) {
    ...
    // Fallback: DB check
    const res = await pool.query('SELECT device_secret FROM devices WHERE device_id = $1', [creds.deviceId]);
    if (!res.rows.length) return false;
    return safeEqual(res.rows[0].device_secret, creds.deviceSecret || '');
}
