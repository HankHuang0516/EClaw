/**
 * Notification endpoint validation tests (Jest + Supertest)
 *
 * Tests input validation for notification CRUD endpoints.
 * Coverage: GET /api/notifications, GET /count, POST /read, POST /read-all,
...
describe('GET /api/notifications', () => {
    ...
    it('rejects when deviceSecret is missing', async () => {
        const res = await get('/api/notifications?deviceId=dev-1');
        expect(res.status).toBeGreaterThanOrEqual(400);
    });
...
describe('POST /api/notifications/read', () => {
    ...
    it('rejects when notification id is missing', async () => {
        const res = await post('/api/notifications/read')
            .send({ deviceId: 'dev-1', deviceSecret: 'sec' });
        // Should fail auth (device not found) or validation (missing id)
        expect(res.status).toBeGreaterThanOrEqual(400);
    });
});
...
describe('GET /api/notification-preferences', () => {
    it('rejects when deviceId is missing', async () => {
        const res = await get('/api/notification-preferences');
        expect(res.status).toBeGreaterThanOrEqual(400);
    });
});
...
describe('PUT /api/notification-preferences', () => {
    it('rejects when deviceId is missing', async () => {
        const res = await request(app).put('/api/notification-preferences')
            .set('Host', 'localhost')
            .send({ prefs: { bot_reply: false } });
        expect(res.status).toBeGreaterThanOrEqual(400);
    });
});
