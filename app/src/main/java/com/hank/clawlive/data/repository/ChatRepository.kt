...
class ChatRepository private constructor(
    ...
    suspend fun processEntityMessage(entity: EntityStatus) {
        ...
        val isA2APrefix = entity.message.matches(Regex("^entity:\\d+:[^:]+:.*", RegexOption.DOT_MATCHES_ALL))
        if (isA2APrefix) {
            Timber.d("[A2A_POLL_RECV] Entity${entity.entityId}.message has A2A prefix: ${entity.message.take(80)}")
            Timber.d("[A2A_POLL_ENTITY_SKIP] Skipping entity-to-entity msg on Entity${entity.entityId} (handled by syncFromBackend)")
            return
        }
    ...
    suspend fun syncFromBackend(messages: List<ChatHistoryMessage>): Int {
        ...
        for (msg in messages) {
            ...
            if (entityMatch == null && msg.source.startsWith("entity:")) {
                Timber.w("[A2A_SYNC_MISS] source='${msg.source}' did NOT match entityPattern — will be treated as regular msg | id=${msg.id} text=${msg.text.take(40)}")
            }
